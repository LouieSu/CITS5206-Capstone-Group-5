...
# First: find the MIT-2025 ruleset
target_ruleset = None
...
def extract_required_units(section_code):
    return {
        code.strip()
        for section in target_ruleset.findall(".//section") + target_ruleset.findall(".//specialisation")
...
if df.empty:
    print("✅ No validation issues found. Timetable satisfies MIT-2025 rules.")
    print("🎯 Completed Specialisation(s):", ", ".join(completed_specs).upper())
else:
    print("Validation Results:\n")
    print(df.to_string(index=False))

print("Taken Option A units:", taken_option_a)
print("Level 5 Option A units:", [u for u in taken_option_a if get_unit_level(u) == 5])
